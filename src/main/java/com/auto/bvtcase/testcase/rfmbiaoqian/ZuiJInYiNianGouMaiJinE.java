...
public class ZuiJInYiNianGouMaiJinE {
    String token = GetToken.token();
    @BeforeMethod(
            description = "最后一年购买金额"
    )
    public void beforeMethod(){
        TestConfig.calcNumberurl= ConfigFile.getUrl("prod.uri", InterfaceName.calcNumber);}

    @Test(description = "最后一年购买金额")
    public void testGetApi() throws SQLException {
        Integer api = JsonParseUtil.parseType(JsonParseUtil.parseJson(RequestApi.getApi("src/main/java/com/auto/data/rfmbiaoqian/最后一年购买金额.json",token)).get("data").toString());
